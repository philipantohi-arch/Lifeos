 ...
 *
 * Generates candidate actions from today's actual state, scored by
 * predicted Life Score impact — but every threshold, time, and framing is
 * personal: sleep windows come from chronotype, budgets from income, step
 * targets from age, training from fitness level and recovery, and the
 * user's current *situation* (sick, new baby, crunch week, travel, injury)
 * reshapes the whole list. Tone follows self-determination theory
 * (sdt-motivation): explain why, offer choices, never command.
 *
 * Behavior-change guardrail: at most `simultaneousHabitLimit` NEW habits
 * are pushed at once (one-habit-at-a-time); the rest is maintenance.
 */

import type { DayRecord, PersonalTargets, RecommendedAction, UserProfile } from './types';
      ...
      return [
        {
          ...
          because: 'Fragmented-sleep harm reduction for new parents — the target is coping, not optimizing.',
        },
        {
          id: 'baby-tagteam',
          title: 'Ask your partner to take one overnight block',
          detail: 'Alternating one 3–4h protected block each gives you both at least some consolidated sleep — better than both being half-awake all night.',
          impactPoints: 2.6,
          pillar: 'relationships',
          prediction: 'Protected blocks are the single best-evidenced strategy for new-parent sleep.',
...
export function recommendActions(
  ...
  if (!situationBlocksTraining) {
    ...
    // 65+: strength & balance are fall-prevention, not aesthetics.
  ...
  if (spend7 > t.weeklyDiscretionary * 0.92) {
    candidates.push({
      ...
      impactPoints: 2.4,
      pillar: 'wealth',
      prediction: `Holding budget this week keeps you on pace for your ${profile.savingsGoalLabel.toLowerCase()} goal.`,
  ...
  // ── Relationships: personalized cadence (in-person weighted for 65+) ─
  if (today.daysSinceFamilyContact >= t.familyContactCadenceDays) {
    if (profile.age >= 65) {
      candidates.push({
        id: 'see-someone',
        title: 'See someone in person today — coffee, a walk, anything',
        detail: `It's been ${today.daysSinceFamilyContact} days since meaningful contact, past your ${t.familyContactCadenceDays}-day cadence. For your age group, in-person contact 3×/week roughly halves the risk of depression two years out — and in-person beats phone calls in the data.`,
        impactPoints: 2.9,
        pillar: 'relationships',
        prediction: 'Regular in-person contact is one of the strongest health levers available at any price.',
        because: `${today.daysSinceFamilyContact} days vs your ${t.familyContactCadenceDays}-day cadence (Teo 2015 dose-response; Surgeon General advisory).`,
      });
    } else {
      candidates.push({
        id: 'call-family',
        title: 'Call your parents today',
        detail: `It's been ${today.daysSinceFamilyContact} days — past your ${t.familyContactCadenceDays}-day cadence. Strong ties are a ~50% survival advantage over time; this is not a soft metric.`,
        impactPoints: 2.8,
        pillar: 'relationships',
        prediction: 'A 20-minute call today restores a trend line that has been quietly declining.',
        because: `${today.daysSinceFamilyContact} days vs your ${t.familyContactCadenceDays}-day cadence (Holt-Lunstad meta-analysis; Harvard Study of Adult Development).`,
      });
    }
  }
  ...
  // ── Habits: meal prep window (Sundays), nature dose ──────────────────
  ...
  if (drinks7 > t.maxDrinksWeekly) {
    candidates.push({
      ...
      because: `Weekly total exceeds the ${profile.sex === 'female' ? 'female' : 'male'} low-risk ceiling (US Dietary Guidelines; alcohol–sleep field studies).`,
    });
  }

  // ── Behavior-change guardrail: cap NEW habit pushes ──────────────────
